...
#[derive(Clone)]
pub struct EcbCs3Enc<C: BlockEncrypt> {
    cipher: C,
}

impl<C: BlockEncrypt> InnerUser for EcbCs3Enc<C> {
    type Inner = C;
}

impl<C: BlockEncrypt> IvSizeUser for EcbCs3Enc<C> {
    type IvSize = C::BlockSize;
}

impl<C: BlockEncrypt> InnerInit for EcbCs3Enc<C> {
    fn inner_init(cipher: Self::Inner) -> Self {
        Self { cipher }
    }
}

impl<C: BlockEncrypt> Encrypt for EcbCs3Enc<C> {
