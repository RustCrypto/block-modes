    ...
    Chunks<B>: ArrayLength<u128>,
{
    ...
    fn increment(block: &mut GenericArray<u8, B>) {
        let s = u128::from_be_bytes(block[..CS].try_into().unwrap()) + 1;
        block.copy_from_slice(&s.to_be_bytes());
    }
}
/// 128-bit big endian counter flavor.
pub enum Ctr128LE {}
    ...
    Chunks<B>: ArrayLength<u128>,
{
    ...
    #[inline]
    fn increment(block: &mut GenericArray<u8, B>) {
        let s = u128::from_le_bytes(block[..CS].try_into().unwrap()) + 1;
        block.copy_from_slice(&s.to_le_bytes());
    }
}
